...
export default async function OrderDetailPage({ params, searchParams }: OrderDetailPageProps) {
    ...
    return (
      <main className="min-h-screen bg-[#f6f4ef] text-[#1a2f27] px-4 py-8">
        <section className="mx-auto max-w-3xl rounded-3xl border border-[#d8ddd3] bg-white p-6 shadow-sm">
          <p className="text-sm font-semibold text-[#8e3a30]">휴대폰 번호가 필요합니다.</p>
          <Link href="/orders/lookup" className="mt-3 inline-block text-sm font-bold text-[#166847]">
            주문 조회로 이동
          </Link>
        </section>
      </main>
  ...
  if (!order) {
    return (
      <main className="min-h-screen bg-[#f6f4ef] text-[#1a2f27] px-4 py-8">
        <section className="mx-auto max-w-3xl rounded-3xl border border-[#d8ddd3] bg-white p-6 shadow-sm">
          <p className="text-sm font-semibold text-[#8e3a30]">주문을 찾을 수 없습니다.</p>
          <Link href="/orders/lookup" className="mt-3 inline-block text-sm font-bold text-[#166847]">
            주문 조회로 이동
          </Link>
        </section>
    ...
    <main className="min-h-screen bg-[#f6f4ef] text-[#1a2f27] px-4 py-8">
      <section className="mx-auto max-w-3xl rounded-3xl border border-[#d8ddd3] bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/orders/lookup" className="text-sm font-bold text-[#166847]">
            조회로
          </Link>
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-[#d8ddd3] bg-[#f9fcfa] p-4">
          ...
          <div className="mt-2 flex justify-between border-t border-[#d8ddd3] pt-2">
            <span className="font-bold">예상 총액</span>
            <strong className="font-black text-[#145c3f]">{formatPrice(order.total_estimated)}</strong>
          </div>
        </div>

        <ul className="mt-4 space-y-2 rounded-2xl border border-[#d8ddd3] bg-[#f9fcfa] p-4 text-sm">
